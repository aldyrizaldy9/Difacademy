...
public class OpOnlineMateriActivity extends AppCompatActivity {
    ...
    ArrayList<OnlineMateriModel> onlineMateriModels;
    OpOnlineMateriAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        onlineMateriModels = new ArrayList<>();
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        OnlineMateriModel model = intent.getParcelableExtra("online_materi_model");
        int index = intent.getIntExtra("index", -1);

        if (requestCode == ADD_REQUEST_CODE && resultCode == RESULT_OK) {
            if (model != null) {
                onlineMateriModels.add(model);
            }
        } else if (requestCode == DELETE_REQUEST_CODE && resultCode == RESULT_OK) {
            if (index != -1) {
                onlineMateriModels.remove(index);
            }
        } else if (requestCode == UPDATE_REQUEST_CODE && resultCode == RESULT_OK) {
            if (model != null) {
                onlineMateriModels.set(index, model);
            }
        }
    ...
    private void setRecyclerView(){
        ...
        adapter = new OpOnlineMateriAdapter(this, onlineMateriModels);
        ...
        rvOnlineMateri.addOnScrollListener(new RecyclerView.OnScrollListener() {
            ...
            public void onScrollStateChanged(@NonNull RecyclerView recyclerView, int newState) {
                ...
                if (manager.findLastVisibleItemPosition() >= onlineMateriModels.size() - 10 &&
                        lastVisible != null &&
                        loadbaru) {
                    loadbaru = false;
                                ...
                                public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                                    if (queryDocumentSnapshots.size() > 0) {
                                        for (QueryDocumentSnapshot documentSnapshot : queryDocumentSnapshots) {
                                            OnlineMateriModel newModel = documentSnapshot.toObject(OnlineMateriModel.class);
                                            newModel.setDocumentId(documentSnapshot.getId());
                                            onlineMateriModels.add(newModel);
                                        }
